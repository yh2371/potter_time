...
def train(config, train_loader, model, criterion, optimizer, epoch, output_dir, device, logger, writer_dict):
    ...
    for i, (input, pose_2d_gt, hm_gt, target_weight, pose_3d_gt, vis_flag, wrist, intrinsic, joint_view) in enumerate(train_loader):
        # compute output
        ...
        bs, T, joints, dim = pose_3d_pred.shape
        #print(pose_3d_pred.shape)
        pose_3d_pred = pose_3d_pred.contiguous().view((-1,joints,dim))
        pose_3d_gt = pose_3d_gt.contiguous().view((-1,joints,dim))
        vis_flag = vis_flag.contiguous().view((-1, joints))
        
        bs, T, joints, dimH, dimW = hm_pred.shape
        #print(hm_pred.shape)
        hm_pred = hm_pred.contiguous().view((-1,joints, dimH, dimW))
        hm_gt = hm_gt.contiguous().view((-1,joints, dimH, dimW))
...
def validate(config, val_loader, model, criterion, output_dir, device, logger, writer_dict):
    ...
    with torch.no_grad():
        ...
        for i, (input, pose_2d_gt, hm_gt, target_weight, pose_3d_gt, vis_flag, wrist, intrinsics, _) in enumerate(val_loader):
            # compute output
            input = input.to(device)
            hm_pred, pose_3d_pred = model(input)
            #print(hm_pred.shape, pose_3d_pred.shape)
            hm_gt = hm_gt.to(device)
            # Assign None kpts as zero
            pose_3d_gt[~vis_flag] = 0
            pose_3d_gt = pose_3d_gt.to(device)
...
def main(args):
    ...
    device = torch.device(f'cuda:{3}' if torch.cuda.is_available() else 'cpu') #args.gpu_number[0]
