...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=16, stride=16, padding=0,
        ...
        patch_size = (3, patch_size[1], patch_size[2])
        print(("kernel", patch_size))
...
class PoolAttn(nn.Module):
    ...
    def __init__(self, dim=256, norm_layer=GroupNorm):
        ...
        self.patch_pool1 = nn.AdaptiveAvgPool3d((4, 4, None)) #_ x _ x T
        self.patch_pool2 = nn.AdaptiveAvgPool3d((4, None, 4)) #_ x W x 
        self.patch_pool3 = nn.AdaptiveAvgPool3d((None, 4, 4)) # H x _ x _

        self.embdim_pool1 = nn.AdaptiveAvgPool3d((4, 4, None)) #_ x _ x T
        self.embdim_pool2 = nn.AdaptiveAvgPool3d((4, None, 4)) #_ x DW x 
        self.embdim_pool3 = nn.AdaptiveAvgPool3d((None, 4, 4)) # DH x _ x _

        # self.act = act_layer()
        self.norm = norm_layer(dim)
        # self.proj = nn.Conv2d(dim,dim,1)
        self.proj0 = nn.Conv3d(dim, dim, 3, 1, 1, bias=True, groups=dim) #Changed to 3D
        self.proj1 = nn.Conv3d(dim, dim, 3, 1, 1, bias=True, groups=dim)
        self.proj2 = nn.Conv3d(dim, dim, 3, 1, 1, bias=True, groups=dim)

    def forward(self, x):
        B, C, T, H, W = x.shape
        x = x.permute(0,1,3,4,2) #B, C, H, W, T
        x_patch_attn1 = self.patch_pool1(x) # B, C, 4, 4, T
        x_patch_attn2 = self.patch_pool2(x) # B, C, 4, W, 4
        x_patch_attn3 = self.patch_pool3(x).permute(0,1,4,2,3) # B, C, 4, H, 4

        x_patch_attn_pt1 = x_patch_attn2 @ x_patch_attn1 # B, C, 4, W, T
        x_patch_attn_pt2 = x_patch_attn3 @ x_patch_attn1 # B, C, 4, H, T

        x_patch_attn_pt1 = x_patch_attn_pt1.permute(0,1,4,2,3) # B, C, T, 4, W
        x_patch_attn_pt2 = x_patch_attn_pt2.permute(0,1,4,3,2) # B, C, T, H, 4
        x_patch_attn = x_patch_attn_pt2 @ x_patch_attn_pt1 # B, C, T, H, W
        x_patch_attn = self.proj0(x_patch_attn)

        x1 = x.permute(0,4,1,2,3).contiguous().view(B, T, C, H * W).transpose(2, 3).contiguous().view(B, T, H * W, 32, -1).permute(0, 2, 3, 4, 1) #B, HW, DH, DW, T
        x_embdim_attn1 = self.embdim_pool1(x1) # B, HW, 4, 4, T
        x_embdim_attn2 = self.embdim_pool2(x1) # B, HW, 4, DW, 4
        x_embdim_attn3 = self.embdim_pool3(x1).permute(0,1,4,2,3) # B, HW, 4, DH, 4
        x_embdim_attn_pt1 = x_embdim_attn2 @ x_embdim_attn1 # B, HW, 4, DW, T
        x_embdim_attn_pt2 = x_embdim_attn3 @ x_embdim_attn1 # B, HW, 4, DH, T

        x_embdim_attn_pt1 = x_embdim_attn_pt1.permute(0,1,4,2,3) # B, HW, T, 4, Dw
        x_embdim_attn_pt2 = x_embdim_attn_pt2.permute(0,1,4,3,2) # B, HW, T, DH, 4 
        x_embdim_attn = x_embdim_attn_pt2 @ x_embdim_attn_pt1 # B, HW, T, DH, DW
        x_embdim_attn = x_embdim_attn.contiguous().view(B, H * W, T, C).permute(0, 3, 2, 1).contiguous().view(B, C, T, H, W)
        x_embdim_attn = self.proj1(x_embdim_attn)

        x_out = self.norm(x_patch_attn + x_embdim_attn)
        x_out = self.proj2(x_out) # B, C, T, H, W
        return x_out
...
class PoolFormerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(self.layer_scale_2.unsqueeze(-1).unsqueeze(-1).shape)
...
class PatchSplit(nn.Module):
    ...
    def __init__(self, stride=2,
        ...
        super().__init__()
        self.stride = stride
        self.upsample = nn.Upsample(scale_factor=(1,stride,stride), mode='nearest')
        self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=3,
                              stride=1, padding=1)
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()
...
class HR_stream(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("1", self.patch_emb1(x1).shape)
        # print(x1_new.shape)

        # print("2", self.patch_emb2(x2).shape)
